...
const [bcbSelic, tesouroData] = await Promise.all([
    bcbSelicRequest,
    tesouroDataRequest,
])
const taxaSelic = (bcbSelic.data[0].valor) * (10 ** 6);
const filteredArray = tesouroData.data.response.TrsrBdTradgList.filter(item => item.TrsrBd.cd === 178); //CD for Tesouro Selic 2029
