...
async function deployTokensFixture(deployer, investor, investor2) {
	...
	await drexToken
		.connect(deployer)
		.transfer(investor.address, ethers.utils.parseUnits("1000", 6))
...
async function deployUniPoolFixture(deployer, drexToken, tselicToken) {
	...
	const UniFactory = new ethers.ContractFactory(artifacts.UniswapV3Factory.abi, artifacts.UniswapV3Factory.bytecode, deployer);
	let uniFactory = await UniFactory.deploy();
	await uniFactory.deployed();

	const SwapRouter = new ethers.ContractFactory(artifacts.SwapRouter.abi, artifacts.SwapRouter.bytecode, deployer);
	let swapRouter = await SwapRouter.deploy(uniFactory.address, weth.address);
	await swapRouter.deployed();

	const NFTDescriptor = new ethers.ContractFactory(artifacts.NFTDescriptor.abi, artifacts.NFTDescriptor.bytecode, deployer);
	...
	const NonfungibleTokenPositionDescriptor = new ethers.ContractFactory(artifacts.NonfungibleTokenPositionDescriptor.abi, linkedBytecode, deployer);
	...
	const NonfungiblePositionManager = new ethers.ContractFactory(artifacts.NonfungiblePositionManager.abi, artifacts.NonfungiblePositionManager.bytecode, deployer);
	let nonfungiblePositionManager = await NonfungiblePositionManager.deploy(uniFactory.address, weth.address, nonfungibleTokenPositionDescriptor.address);
	...
	const poolAddress = await uniFactory.connect(deployer).getPool(
		tselicToken.address,
		drexToken.address,
		3000,
	)

	tselicToken.connect(deployer).approve(nonfungiblePositionManager.address, ethers.utils.parseUnits('1000000', 18))
	drexToken.connect(deployer).approve(nonfungiblePositionManager.address, ethers.utils.parseUnits('1000000', 6))
	...
	const tx = await nonfungiblePositionManager.connect(deployer).mint(
		params,
		{ gasLimit: '1000000' }
	)
	await tx.wait()
